...
$(() => {
            ...
            if (data) {
                const replacements = {
                    ...
                    __bathroom__:
                        ...
                        "0",
                    __square__:
                        data.square || data.square_text || data.area || "",
                };
            ...
            const observer = new MutationObserver((mutations) => {
                ...
                mutations.forEach((mutation) => {
                    $(mutation.addedNodes).each(function () {
                        if (
                            $(this).hasClass("map-listing-item") ||
                            $(this).find(".map-listing-item").length > 0
                        ) {
    ...
    //     if (
    //         $counter.length > 0 &&
    //         $(document.body).hasClass("counter-scroll")
    //     ) {
    //         let a = 0;
    ...
    const clickSearchForm = function () {
        ...
        if (widgetSearchForm.length) {
            ...
            $(document).on(
                "click",
                ".pull-right, .offcanvas-backdrop",
                function (a) {
                    a.preventDefault();
                    if (
                        $(a.target).closest(".pull-right, .wd-search-form")
                            .length === 0
                    ) {
            ...
            if (nav.length) {
                ...
                $(window).on("load scroll", function () {
                    if ($(window).scrollTop() > 0) {
                        ...
                        $("#trans-logo").attr(
                            "src",
                            "images/logo/logo-footer@2x.png",
                        );
                    }
                });
            }
    ...
    const animateHeading = () => {
        ...
        function hideWord($word) {
            ...
            if ($word.parents(".animationtext").hasClass("type")) {
                ...
                $word.parents(".animationtext").hasClass("loading-bar")
            ) {
    ...
    const rangeSlider = () => {
        ...
        const priceSlider = () => {
            ...
            $('[data-bb-toggle="range"]').each((index, el) => {
                ...
                const currencySymbol =
                    $(rangeSlider).data("currency-symbol") || "$";
                ...
                const currencyWithSpace = $(rangeSlider).data(
                    "currency-with-space",
                );
                ...
                rangeSlider.noUiSlider.on("update", function (values, handle) {
                    $element
                        .find('[data-bb-toggle="range-from-value"]')
                        .html(values[0]);
                    $element
                        .find('[data-bb-toggle="range-to-value"]')
                        .html(values[1]);
                });
        ...
        const flatSlider = () => {
            ...
            rangeSlider.noUiSlider.on("update", function (values, handle) {
                document.getElementById("slider-flat-value01").innerHTML =
                    values[0];
                document.getElementById("slider-flat-value02").innerHTML =
                    values[1];
            });
        ...
        $(document)
            ...
            .on(
                ...
                (e) => {
                    ...
                    const parentTab = currentTarget.closest(
                        ".flat-tab-recommended",
                    );
                    ...
                    $.ajax({
                        ...
                        error: (error) => Theme.handleError(error),
                        complete: () =>
                            parentTab.find(".loading-spinner").remove(),
                    });
                },
            );
    };
                ...
                $.ajax({
                    ...
                    success: ({ data, meta }) => {
                        ...
                        data.forEach((item) => {
                            ...
                            content = content
                                .replace(
                                    new RegExp("__name__", "gi"),
                                    item.name,
                                )
                                .replace(
                                    new RegExp("__location__", "gi"),
                                    item.location,
                                )
                                .replace(
                                    new RegExp("__image__", "gi"),
                                    item.image_thumb,
                                )
                                .replace(
                                    new RegExp("__price__", "gi"),
                                    item.formatted_price,
                                )
                                .replace(new RegExp("__url__", "gi"), item.url)
                                .replace(
                                    new RegExp("__status__", "gi"),
                                    item.status_html,
                                );
                            ...
                            if (isProperty) {
                                ...
                                if (
                                    !item.number_bedroom ||
                                    item.number_bedroom === 0
                                ) {
                                    content = content.replace(
                                        /<li class="map-number-of-bedroom">[\s\S]*?<\/li>/gi,
                                        "",
                                    );
                                } else {
                                    content = content.replace(
                                        new RegExp("__bedroom__", "gi"),
                                        item.number_bedroom,
                                    );
                                }
                                ...
                                if (
                                    ...
                                    item.number_bathroom === 0
                                ) {
                                    content = content.replace(
                                        /<li class="map-number-of-bathroom">[\s\S]*?<\/li>/gi,
                                        "",
                                    );
                                } else {
                                    content = content.replace(
                                ...
                                if (
                                    ...
                                    content = content.replace(
                                        /<li class="map-square">[\s\S]*?<\/li>/gi,
                                        "",
                                    );
                                } else {
                                    content = content.replace(
                                        new RegExp("__square__", "gi"),
                                        item.square_text,
                                    );
                                }
                            }
    ...
    const initWishlistCount = () => {
        ...
        const projectWishlist = decodeURIComponent(
            getCookie("project_wishlist") || "",
        );

        const wishlistArray = wishlist ? wishlist.split(",") : [];
        const projectWishlistArray = projectWishlist
            ? projectWishlist.split(",")
            : [];

        $('[data-bb-toggle="wishlist-count"]').text(
            wishlistArray.length + projectWishlistArray.length,
        );
    };
    ...
    $(document)
        ...
        .on("submit", "#hero-search-form", function (e) {
            ...
            window.location.href =
                $form.prop("action") + cleanedFormData.queryString;
        })
        .on(
            "keyup",
        ...
        .on("click", (e) => {
            if (
                !$(e.target).closest('[data-bb-toggle="data-suggestion"]')
                    .length
            ) {
        ...
        .on("click", '[data-bb-toggle="add-to-wishlist"]', (e) => {
            ...
            const cookieName =
                $currentTarget.data("type") === "property"
                    ? "wishlist"
                    : "project_wishlist";
    ...
    const enhancedLocationDropdown = function () {
        ...
        function enhanceLocationDropdown() {
            ...
            if (
                !$locationNiceSelect.find(".nice-select-search-wrapper").length
            ) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (typeof response === "string") {
                        ...
                        $tempElement
                            .find(".search-suggestion-item")
                            .each(function () {
                                const cityFullName = $(this).text().trim();
                                const cityId =
                                    $(this).data("value") ||
                                    generateTempId(cityFullName);
                        ...
                        allItemsLoaded =
                            cities.length === 0 || cities.length < 10;
                    } else {
                        ...
                        if (
                            ...
                            allItemsLoaded =
                                !response.data.has_more || cities.length === 0;
                        } else {
                            allItemsLoaded = true;
                        }
                    }
        ...
        setTimeout(function () {
            if (
                !$("#location").next(".nice-select").length &&
                $.fn.niceSelect
            ) {
                $("#location").niceSelect();
                enhanceLocationDropdown();
